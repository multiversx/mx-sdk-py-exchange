...
from utils.utils_chain import nominated_amount, WrapperAddress as Address
...
def _withdraw_seeded_rewards(
    ...
    for attempt in range(3):
        current_remaining = _remaining_uncollected_rewards(staking_contract, network_providers.proxy)
...
def seed_staking_rewards(
    ...
    remaining_before = _remaining_uncollected_rewards(staking_contract, network_providers.proxy)
    minimum_remaining = nominated_amount(10_000_000)
    topup_amount = nominated_amount(50_000_000)
    farming_token = staking_contract.farming_token

    if remaining_before < minimum_remaining:
        _ensure_deployer_has_egld(deployer_account, test_environment, network_providers)
