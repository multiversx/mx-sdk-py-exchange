...
class PriceDiscoveryEconomics:
    ...
    def __check_deposit_event(self, event: DepositPDLiquidityEvent):
        ...
        if chain_first_token_reserve != new_first_token_reserve:
            print_test_step_fail("TEST CHECK FAIL: First token reserve not as expected!")
            print_test_substep(f"Chain first token reserve: {chain_first_token_reserve}")
            print_test_substep(f"Expected first token reserve: {new_first_token_reserve}")

        if chain_second_token_reserve != new_second_token_reserve:
            print_test_step_fail("TEST CHECK FAIL: Second token reserve not as expected!")
            print_test_substep(f"Chain second token reserve: {chain_second_token_reserve}")
            print_test_substep(f"Expected second token reserve: {new_second_token_reserve}")

        print_test_step_pass("Checked deposit event data!")
    ...
    def __deposit_event_account_tracking(self, event: DepositPDLiquidityEvent, user_address: Address):
        ...
        for chain_token in chain_tokens_on_account:
            if chain_token['nonce'] == self.pd_contract_identity.first_redeem_token_nonce:
                ...
                if chain_first_redeem_tokens != exp_first_redeem_tokens_owned:
                    print_test_step_fail("TEST CHECK FAIL: First redeem tokens on account not as expected!")
                    print_test_substep(f"Chain first redeem tokens: {chain_first_redeem_tokens}")
                    print_test_substep(f"Expected first redeem tokens: {exp_first_redeem_tokens_owned}")
            ...
            if chain_token['nonce'] == self.pd_contract_identity.second_redeem_token_nonce:
                ...
                if chain_second_redeem_tokens != exp_second_redeem_tokens_owned:
                    print_test_step_fail("TEST CHECK FAIL: Second redeem tokens on account not as expected!")
                    print_test_substep(f"Chain second redeem tokens: {chain_second_redeem_tokens}")
                    print_test_substep(f"Expected second redeem tokens: {exp_second_redeem_tokens_owned}")
                    second_redeem_token_found = True

        if not first_redeem_token_found and exp_first_redeem_tokens_owned > 0:
            print_test_step_fail("TEST CHECK FAIL: First redeem tokens on account not as expected!")
            print_test_substep(f"Chain first redeem tokens: 0")
            print_test_substep(f"Expected first redeem tokens: {exp_first_redeem_tokens_owned}")
        if not second_redeem_token_found and exp_second_redeem_tokens_owned > 0:
            print_test_step_fail("TEST CHECK FAIL: Second redeem tokens on account not as expected!")
            print_test_substep(f"Chain second redeem tokens: 0")
            print_test_substep(f"Expected second redeem tokens: {exp_second_redeem_tokens_owned}")
        ...
        print_test_step_pass("Tracked and checked deposit account data!")
    ...
    def deposit_event_tracking(self, event: DepositPDLiquidityEvent, user_address: Address, tx_hash: str):
        ...
        print_test_step_pass("Tracked deposit data!")
    ...
    def __check_withdraw_event(self, event: WithdrawPDLiquidityEvent):
        ...
        print_test_step_pass("Checked withdraw event data!")
    ...
    def __withdraw_event_account_tracking(self, event: WithdrawPDLiquidityEvent, user_address: Address):
        ...
        print_test_step_pass("Tracked and checked withdraw account data!")
    ...
    def __check_redeem_event(self, event: RedeemPDLPTokensEvent):
        ...
        if chain_lp_tokens_reserve != exp_lp_tokens_reserve:
            print_test_step_fail("TEST CHECK FAIL: LP tokens reserve not as expected!")
            print_test_substep(f"Chain LP tokens reserve: {chain_lp_tokens_reserve}")
            print_test_substep(f"Expected LP tokens reserve: {exp_lp_tokens_reserve}")

        # update LP tokens tracking data
        self.lp_tokens_reserve = chain_lp_tokens_reserve

        print_test_step_pass("Checked redeem event data!")
    ...
    def __redeem_event_account_tracking(self, event: RedeemPDLPTokensEvent, user_address: Address):
        ...
        if chain_lp_tokens_on_account != exp_lp_tokens_owned:
            print_test_step_fail("TEST CHECK FAIL: LP tokens on account not as expected!")
            print_test_substep(f"Chain LP tokens on account: {chain_lp_tokens_on_account}")
            print_test_substep(f"Expected LP on account: {exp_lp_tokens_owned}")
        ...
        print_test_step_pass("Tracked and checked redeem account data!")
