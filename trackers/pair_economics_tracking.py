...
class PairEconomics(Subscriber):
    ...
    def check_add_liquidity(self, event: AddLiquidityEvent):
        ...
        if old_first_token_reserve >= new_first_token_reserve:
            print_test_step_fail(f'First token reserve did not increase for pair {self.contract_address.bech32()}')
            print_test_substep(f'Old first token reserve: {old_first_token_reserve}')
            print_test_substep(f'New first token reserve: {new_first_token_reserve}')
            print_test_substep(f'Minimum first token reserve expected: {expected_first_token_reserve}')

        if old_second_token_reserve >= new_second_token_reserve:
            print_test_step_fail(f'Second token reserve did not increase for pair {self.contract_address.bech32()}')
            print_test_substep(f'Old second token reserve: {old_second_token_reserve}')
            print_test_substep(f'New second token reserve: {new_second_token_reserve}')
            print_test_substep(f'Minimum second token reserve expected: {expected_second_token_reserve}')

        if old_total_supply >= new_total_supply:
            print_test_step_fail(f'Total supply did not increase for pair {self.contract_address.bech32()}')
            print_test_substep(f'Old supply: {old_total_supply}')
            print_test_substep(f'New supply: {new_total_supply}')

        print_test_step_pass('Checked addLiquidityEvent economics!')
    ...
    def check_remove_liquidity(self, event: RemoveLiquidityEvent):
        ...
        if old_first_token_reserve <= new_first_token_reserve:
            print_test_step_fail(f'First token reserve did not decrease for pair {self.contract_address.bech32()}')
            print_test_substep(f'Old first token reserve: {old_first_token_reserve}')
            print_test_substep(f'New first token reserve: {new_first_token_reserve}')
            print_test_substep(f'Maximum first token reserve expected: {expected_first_token_reserve}')

        if old_second_token_reserve <= new_second_token_reserve:
            print_test_step_fail(f'Second token reserve did not decrease for pair {self.contract_address.bech32()}')
            print_test_substep(f'Old second token reserve: {old_second_token_reserve}')
            print_test_substep(f'New second token reserve: {new_second_token_reserve}')
            print_test_substep(f'Maximum second token reserve expected: {expected_second_token_reserve}')

        if old_total_supply <= new_total_supply:
            print_test_step_fail(f'Total supply did not decrease for pair {self.contract_address.bech32()}')
            print_test_substep(f'Old supply: {old_total_supply}')
            print_test_substep(f'New supply: {new_total_supply}')

        print_test_step_pass('Checked removeLiquidityEvent economics!')
    ...
    def check_swap_fixed_input(self, event: SwapFixedInputEvent):
        ...
        if event.tokenA == self.first_token:
            ...
            if old_first_token_reserve >= new_first_token_reserve:
                ...
                print_test_substep(f'Old first token reserve: {old_first_token_reserve}')
                print_test_substep(f'New first token reserve: {new_first_token_reserve}')
                print_test_substep(f'Expected first token reserve: {expected_first_token_reserve}')

            if old_second_token_reserve <= new_second_token_reserve:
                print_test_step_fail(f'Second token reserve did not decrease for pair {self.contract_address.bech32()}')
                print_test_substep(f'Old second token reserve: {old_second_token_reserve}')
                print_test_substep(f'New second token reserve: {new_second_token_reserve}')
                print_test_substep(f'Maximum expected second token reserve: {expected_second_token_reserve}')
        else:
            expected_first_token_reserve = old_first_token_reserve - event.amountBmin
            expected_second_token_reserve = old_second_token_reserve + event.amountA
        ...
        print_test_step_pass(f'Checked swapFixedInputEvent economics')
    ...
    def check_swap_fixed_output(self, event: SwapFixedOutputEvent):
        ...
        if event.tokenA == self.first_token:
            ...
            if old_first_token_reserve >= new_first_token_reserve:
                ...
                print_test_substep(f'Old first token reserve: {old_first_token_reserve}')
                print_test_substep(f'New first token reserve: {new_first_token_reserve}')
                print_test_substep(f'Maximum expected first token reserve: {expected_first_token_reserve}')

            if old_second_token_reserve <= new_second_token_reserve:
                print_test_step_fail(f'Second token reserve did not decrease for pair {self.contract_address.bech32()}')
                print_test_substep(f'Old second token reserve: {old_second_token_reserve}')
                print_test_substep(f'New second token reserve: {new_second_token_reserve}')
                print_test_substep(f'Expected second token reserve: {expected_second_token_reserve}')
        else:
            expected_first_token_reserve = old_first_token_reserve - event.amountB
        ...
        print_test_step_pass(f'Checked swapFixedOutputEvent economics')
